...
const Skills: React.FC = () => {
  useEffect(() => {
    ...
    gsap.from("#alwayslearning-container", {
      y: 50,
      opacity: 0,
      duration: 0.8,
      scrollTrigger: {
        trigger: "#alwayslearning-container",
        start: "top 97%",
        end: "top 90%",
      },
    });
  ...
  return (
    <section id="skills" className="section py-20">
      ...
      <div className="mt-16 text-center" id="alwayslearning-container">
        <h3 className="text-2xl font-bold text-white mb-8">Always Learning</h3>
        <div className="flex flex-wrap justify-center gap-4">
          {learningTech.map((skill) => (
            <span
              key={skill}
              className="px-4 py-2 glass rounded-full text-gray-300 hover:text-white hover:scale-105 transition-all duration-300 cursor-pointer learningTechItem"
            >
              {skill}
            </span>
          ))}
